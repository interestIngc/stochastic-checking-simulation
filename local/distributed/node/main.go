...
func main() {
	...
	if e != nil {
		logger.Fatal(fmt.Sprintf("Can't read from file %s", *inputFile))
	}

	byteArray, e := io.ReadAll(iFile)
	if e != nil {
		logger.Fatal(fmt.Sprintf("Could not read bytes from the input file\n%e", e))
	}

	var input Input
	e = json.Unmarshal(byteArray, &input)
	if e != nil {
		logger.Fatal(fmt.Sprintf("Could not parse json from the input file\n%e", e))
	}
	...
	for i, currByte := range strings.Split(*baseIpAddress, ".") {
		...
		if e != nil {
			logger.Fatal(fmt.Sprintf("Byte %d in base ip address is invalid", i))
		}
		if i >= Bytes {
	...
	default:
		logger.Fatal(fmt.Sprintf("Invalid protocol: %s", input.Protocol))
	}
